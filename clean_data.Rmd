...
data_select <- raw_data %>%
  dplyr::select(REGION, AGEP_A, SEX_A, CVDDIAG_A, NATUSBORN_A, RACEALLP_A, MEDNG12M_A,
         PAYWORRY_A, DIBEV_A, ASEV_A, CHLEV_A, HYPEV_A, MEDRXTRT_A,
         HLTHCOND_A, DEPEV_A) %>% 
  mutate(AGE = case_when(AGEP_A <= 30 ~ 1,
                         AGEP_A > 30 & AGEP_A <= 45 ~ 2,
                         AGEP_A > 45 & AGEP_A <= 60 ~ 3,
                         AGEP_A > 60 & AGEP_A <= 84 ~ 4,
                         AGEP_A == 85 ~ 5)) %>%
